...
const Header: React.FC = () => {
    ...
    return (
        <div className={`flex justify-between bg-darkBlue flex-col w-full z-[99]`}>
            <div className="max-w-[1220px] max-md:px-4 flex w-full mx-auto items-center relative">
                <div className="bg-white/10 w-full flex justify-between items-center py-4 lg:pe-10 lg:ps-[88px] max-lg:px-10 lg:mt-4 mt-3 rounded-[60px] border-white/15 border backdrop:blur-xl">
                    <button onClick={() => setOpen(!open)} className={`hidden md:max-w-10 md:h-11 h-full max-w-8 w-full justify-center items-center max-lg:absolute max-lg:right-12 max-md:right-14 relative z-[70] max-lg:flex flex-col overflow-hidden`}>
                    ...
                    <div className={`flex xl:gap-6 xl:pl-10 lg:pl-5 items-center max-lg:px-4 relative w-full max-lg:bg-black mx-auto gap-5 !text-black lg:max-h-max max-lg:fixed max-lg:top-0 max-lg:h-full max-lg:w-full max-lg:flex-col max-lg:bg-hero-pattern bg-cover bg-top max-lg:duration-300 justify-center max-lg:items-center z-[60] ${open ? "max-lg:left-0" : "max-lg:left-full"}`}>
                        {NAV_LINKS_LIST.map((item, index) => (
                            <div key={index} className="relative group"
                                ...
                                <Link href={item.link} onClick={() => handleLinkClick(index, item.link)} className={`relative lg:text-sm text-xl hover:text-lightSkyBlue text-white/70 transition-all duration-300 !tracking-wide ease-linear group
                                    ${index === 4 ? "!text-lightSkyBlue !font-bold" : ""}
                                ...
                                {index === 2 && hoveredIndex === 2 && (
                                    <div className="absolute left-0 top-full pt-2 bg-black transition-all ease-linear duration-300 shadow-lg rounded-xl p-4 w-full">
                                        <p className="text-white/70 cursor-pointer mt-0.5 hover:scale-110 transition-all ease-linear duration-300">link 1</p>
                                        <p className="text-white/70 cursor-pointer mt-0.5 hover:scale-110 transition-all ease-linear duration-300">link 2</p>
                                        <p className="text-white/70 cursor-pointer mt-0.5 hover:scale-110 transition-all ease-linear duration-300">link 3</p>
                                    </div>
                                )}
                            </div>
                        ))}
